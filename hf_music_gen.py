    ...
    for attempt in range(4):
        try:
            ...
            if resp.status_code == 200:
                ...
                print(f"[HF] Model loading, waiting {est:.0f}s... (attempt {attempt+1}/4)")
...
def get_hf_beat(style: str, mood: str, tempo: str, target_bpm: int) -> str | None:
    """
    ...
    result = generate_background_music(style, mood, tempo, target_bpm, duration_seconds=30, output_path=out_path)
    return result
...
def generate_singing_vocal(
    ...
    if not voice_preset:
        ...
        voice_preset = BARK_VOICE_MAP["_fallback_male" if is_male else "_fallback_female"]
...
if __name__ == "__main__":
    ...
    if HF_TOKEN:
        print("\\n🧪 Testing MusicGen...")
        result = generate_background_music("Pop", "Joy", "Medium", 110, duration_seconds=8, output_path="test_beat.wav")
        if result:
